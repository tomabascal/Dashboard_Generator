...
st.markdown(
    "**Upload PPTX Template**  <br><i>(Text Box format that will be edited → {Column Letter} For Example: <code>{A}</code>)</i>",
    unsafe_allow_html=True
)
...
st.markdown(
    "**Upload Excel File**  <br><i>(Column A must be <code>Store ID</code>)</i>",
    unsafe_allow_html=True
)
